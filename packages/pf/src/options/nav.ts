	...
	.object({
		dynamicPages: z
			.object({
				...
				blogPost: slugString.optional().default("/blog/posts/{slug}").register(primaryRegistry, {
					description:
						'the URL template for post pages; This should include the "{slug}" placeholder.',
				}),
				...
				resumeOrg: slugString.optional().default("/resume/orgs/{slug}").register(primaryRegistry, {
					description:
						'the URL template for organisation pages; This should include the "{slug}" placeholder.',
				}),
