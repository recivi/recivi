...
export const faviconSchema = z
	.object({
		...
		isEnabled: z.boolean().optional().default(true).register(primaryRegistry, {
			description: "whether to automatically identify and use favicons on the site",
		}),
		...
		fileNames: z
			.object({
				...
				faviconSvg: z.string().optional().default("favicon.svg").register(primaryRegistry, {
					description: "the name with extension of the SVG version of the favicon",
				}),
				/** the prefix used by manifest icons */
				manifestIconsPrefix: z.string().optional().default("icon-").register(primaryRegistry, {
