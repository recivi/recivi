	...
	return {
		...
		hooks: {
			"astro:config:setup": async (params) => {
				...
				const { site, base, trailingSlash, root, srcDir, publicDir, build } = params.config;
				...
				if (parsedOptions.favicon.isEnabled && projectContext.hasManifestIcons) {
				...
				params.updateConfig({
					markdown: {
						shikiConfig: {
							themes: {
								// Match the default light props.
								light: "catppuccin-latte",
								// Match the default dark props.
			...
			"astro:build:done": async (params) => {
				...
				if (projectContext.site) {
					await captureOgImages(projectContext, params.dir, params.pages, params.logger,);
				}
			},
		},
	};
